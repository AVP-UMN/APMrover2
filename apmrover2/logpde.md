...
Here a **macro** is defined.A macro ( "macroinstruction") is a rule or pattern that specifies how a certain input sequence should be mapped to a replacement output sequence according to a defined procedure.
...
...
```
The macro pass it the capitalised name of the log options without the LOG_ prefix.  It will check for
the bit being set and print the name of the log option to suit.
...
...
```
One `dump_log`is initialized to the `argv[1].i`. `last_log_num`calls `find_last_log()`funtion defined in [Dataflash library](https://github.com/BeaglePilot/ardupilot/blob/master/libraries/DataFlash/DataFlash.h#L32) for searching the last log number.
After that, checks the value of `dump_log`there are three options:
- If the value is -2 then class [DumpPageInfo](https://github.com/BeaglePilot/ardupilot/blob/master/libraries/DataFlash/DataFlash.h#L41) a funtion defined in Dataflash.h  which dumps the information of the page.


- If the value is equal or under 0, the dumps all.


- And if the `dump_log`is not 2 or is greater than `last_log_number`prints an error message.
...
This funtion erases a log, using `EraseAll()` funtion from DataFlash.
